...
# pattern1 for item1, pattern2 for item2, etc.
DEFAULT_PATTERN_ITEM_FILLED = [(p, p, 1) for p in range(1, 6)]
...
def make_cutstock_pattern_generation_model(items, roll_width, **kwargs):
    ...
    gen_model.use_vars = gen_model.integer_var_list(
        keys=items, ub=999999, name='use')
    # 2 setup constraint:
    # --- sum of item usage times item sizes must be less than roll width
    gen_model.add(gen_model.dot(gen_model.use_vars,
                  (it.size for it in items)) <= roll_width)

    # store dual expression for dynamic edition
    gen_model.use_dual_expr = 1 - \
        gen_model.dot(gen_model.use_vars, gen_model.duals)
    # minimize
...
def make_custstock_master_model(item_table, pattern_table, fill_table, roll_width, **kwargs):
    ...
    m.pattern_item_filled = {
        (m.patterns_by_id[p], m.items_by_id[i]): f for (p, i, f) in fill_table}
    m.roll_width = roll_width

    # --- variables
    # one cut var per pattern...
    m.MAX_CUT = 9999
    m.cut_vars = m.continuous_var_dict(
        m.patterns, lb=0, ub=m.MAX_CUT, name="cut")
...
def cutstock_print_solution(cutstock_model):
    ...
    cut_var_values = {
        p: cutstock_model.cut_vars[p].solution_value for p in patterns}
...
def cutstock_solve(item_table, pattern_table, fill_table, roll_width, **kwargs):
    ...
    master_model = make_custstock_master_model(
        item_table, pattern_table, fill_table, roll_width, **kwargs)

    # these two fields contain named tuples
    items = master_model.items
    patterns = master_model.patterns
    gen_model = make_cutstock_pattern_generation_model(
        items, roll_width, **kwargs)
    ...
    while loop_count < 100 and abs(best - curr) >= obj_eps:
        ...
        if not ms:
            ...
            if rc_cost <= -rc_eps:
                if verbose:
                    print('{}> slave model runs with obj={:g}'.format(
                        loop_count, rc_cost))
            else:
                if verbose:
                    print(
                        '{}> pattern-generator model stops, obj={:g}'.format(loop_count, rc_cost))
                break

            use_values = gen_model.solution.get_values(gen_model.use_vars)
            if verbose:
                print('{}> add new pattern to master data: {}'.format(
                    loop_count, str(use_values)))
